...
class SourcesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            // save logo
            if (!empty($this->request->data['Source']['logo']['tmp_name']) && is_uploaded_file($this->request->data['Source']['logo']['tmp_name'])) {
                $path_info = pathinfo($this->request->data['Source']['logo']['name']);
                chmod($this->request->data['Source']['logo']['tmp_name'], 0644);
                $photo = time() . mt_rand() . "." . $path_info['extension'];

                //path to source image directory
                $fullpath = WWW_ROOT . "img/source";
                if (!is_dir($fullpath)) {
                    mkdir($fullpath, 0777, true);
                }

                //save source image
                move_uploaded_file($this->request->data['Source']['logo']['tmp_name'], $fullpath . DS . $photo);
                $this->request->data['Source']['logo'] = $photo;
                $old_path = $fullpath . DS . $photo;
                $new_path = WWW_ROOT . "img/source" . DS . $photo;
    ...
    public function delete()
    {
        ...
        if (!empty($sourceId)) {
            ...
            if ($this->request->isPost() || $this->RequestHandler->isAjax()) {
                //get contact image
                $source = $this->Source->getSourceLogo($sourceId);
                //delete source
                ...
                if ($success) {
                    ...
                    //remove source image
                    $fullpath = WWW_ROOT . "img/source";
                    if (!empty($source['Source']['logo']) && file_exists($fullpath . DS . $source['Source']['logo'])) {
                        unlink($fullpath . DS . $source['Source']['logo']);
                    }
                    //return json success message
