...
class PipelinesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            // save logo
            if (!empty($this->request->data['Pipeline']['logo']['tmp_name']) && is_uploaded_file($this->request->data['Pipeline']['logo']['tmp_name'])) {
                $path_info = pathinfo($this->request->data['Pipeline']['logo']['name']);
                chmod($this->request->data['Pipeline']['logo']['tmp_name'], 0644);
                $photo = time() . mt_rand() . "." . $path_info['extension'];

                //path to pipeline image directory
                $fullpath = WWW_ROOT . "img/pipeline";
                if (!is_dir($fullpath)) {
                    mkdir($fullpath, 0777, true);
                }

                //save pipeline image
                move_uploaded_file($this->request->data['Pipeline']['logo']['tmp_name'], $fullpath . DS . $photo);
                $this->request->data['Pipeline']['logo'] = $photo;
                $old_path = $fullpath . DS . $photo;
                $new_path = WWW_ROOT . "img/pipeline" . DS . $photo;
        ...
        if (!empty($pipelineId)) {
            ...
            if ($this->request->isPost() || $this->RequestHandler->isAjax()) {
                ...
                if ($checkDeal) {
                    ...
                    //get contact image
                    $pipeline = $this->Pipeline->getPipelineLogo($pipelineId);
                    //delete pipeline
                    ...
                    if ($success) {
                        ...
                        $fullpath = WWW_ROOT . "img/pipeline";
                        if (!empty($pipeline['Pipeline']['logo']) && file_exists($fullpath . DS . $pipeline['Pipeline']['logo'])) {
                            unlink($fullpath . DS . $pipeline['Pipeline']['logo']);
                        }
                        //return json success message
